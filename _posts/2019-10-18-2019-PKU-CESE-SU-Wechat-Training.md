...
![新图文的初始设置\|1920px](https://s2.ax1x.com/2019/10/18/KZcn81.png) 
...
3. **封面**。微信封面比例为：首个推送2.35:1，其他推送1:1；分享时总是1:1。如果有可能，尽量提前用Photoshop等软件尝试截取一下封面的内容。请尽量使用分辨率高的图片（下文会提到如何获取高分辨率图片），同时注意图片版权。如图2所示。
4. **基础格式**。基础字号14，行间距1.8倍，页边距 左右8像素。如图3所示。
...
![标题、摘要、封面示例\|429px](https://s2.ax1x.com/2019/10/18/KZckbF.png)

![基础格式示例\|303px](https://s2.ax1x.com/2019/10/18/KZcFDU.png)

### 1.4. 正式开始排版

秀米提供了丰富多样的基础模板，包括“标题”“卡片”“图片”“布局”等选项，每个选项下都提供了各种样式。一些基础模板还提供了文字说明。此外，秀米还会根据时间推出新的时令模板。

模板的使用方式较为简单，在页面左侧选择合适的模板，直接单击，模板内容就会加入到光标所在位置的后面，或是页面的最后。此外，也可以拖动模板到合适的位置，然后松开鼠标。然后，将模板内的文字、图片替换为自己的文字、图片即可。单击模板后，在模板上方会出现工具栏，如图4所示，其中提供了较为丰富的各种操作。
...
![秀米提供的工具栏\|646px](https://s2.ax1x.com/2019/10/18/KZcEE4.png)
...
1. 大段文字需要设置首行缩进2字符的，**不要使用任何形式的空格**。空格在不同设备上显示的效果有很大差异。请在文字段落的工具栏中依次选择“格式”→“首行缩进”设置首行缩进。如图4所示。
...
10. 微信推送中**无法插入除微信推送之外的链接**，如需跳转到其他网页，请使用二维码或将网址放置在“原文链接”栏，并提醒读者。秀米中插入的链接在同步到微信后台后会被删除。
...
推荐配色网站：Color Scheme Designer 3（[http://colorschemedesigner.com/csd-3.5/](http://colorschemedesigner.com/csd-3.5/)，有中文版[http://www.peise.net/tools/web/](http://www.peise.net/tools/web/)）。从色环选择合适的颜色，然后选择配色方案（“单色搭配”“互补色搭配”等），再从“色彩列表”中复制色彩代码，用Word的“替换”功能将代码处理成“#ABCDEF;”的格式，之后就可以导入秀米的配色板了。导入结果如图5所示。

![导入配色板\|302px](https://s2.ax1x.com/2019/10/18/KZcVUJ.png)

对于秀米提供的模板，如果主色调与所选的配色板相差较大，可以自行修改模板的配色，如图6所示。

![修改模板配色\|719px](https://s2.ax1x.com/2019/10/18/KZcZ59.png)
...
![获取视频通用代码\|780px](https://s2.ax1x.com/2019/10/18/KZcmCR.png)

![已调整格式和未调整格式的显示效果\|80px](https://s2.ax1x.com/2019/10/18/KZcugx.png)

如果不想调整格式，可尝试使用HTML代码。在Chrome中新建标签页，按“<kbd>F12</kbd>”调出控制台界面，然后修改HTML代码，形如“`SO<sub>4</sub><sup>2-</sup>`”，再将渲染出的文字复制粘贴到秀米中，如图9所示。

公式的插入目前没有较好的解决方案，只能先用公式编辑器编辑，然后插入图片。

![用HTML代码输入上下标\|1568px](https://s2.ax1x.com/2019/10/18/KZcKv6.png)
